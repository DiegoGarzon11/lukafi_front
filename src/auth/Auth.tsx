			...
			<div className='grid grid-cols-1 lg:grid-cols-2 h-dvh justify-center lg:items-start '>
				<div className='mt-20  w-full  flex  items-center  flex-col md:flex-row    '>
					<div className='flex flex-col md:flex-row  w-full items-end z-10  md:mb-10   '>
						<Link to='/'>
							<button className='p-1 bg-zinc-300 dark:bg-zinc-900 rounded-full mx-5 md:mx-20 flex items-center gap-1 px-5 mb-3 md:mb-0'>
						...
						<div className='flex items-center justify-end mx-5 md:mx-20  '>
							<button
								disabled={view}
								className={`${
									view ? 'dark:text-white font-bold bg-zinc-300 text-black dark:bg-zinc-900' : 'text-black dark:text-white'
								}  text-xl border rounded-l-full p-3 md:w-40 border-zinc-900`}
								onClick={changeAuth}>
								{t('form.field.signIn')}
							</button>
							<button

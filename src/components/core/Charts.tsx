...
const CustomTooltip = ({payload}) => {
	...
	return (
		<div className='custom-tooltip'>
			<p className='payload'>
				{t('dashboard.chart')} {payload[0]?.payload?.date} {t('dashboard.chartSpent')}{' '}
				{Number(payload[0]?.payload?.total_value).toLocaleString()}
			</p>
		</div>
	);
};
	...
	return (
		<ResponsiveContainer height={330}>
			<LineChart
				...
				<Legend />
				<Line activeDot={{r: 8}} type='bump' dataKey='total_value' stroke='red' />
			</LineChart>
		</ResponsiveContainer>
	);
};
...
const showName = (props) => {
	...
	return (
		<g>
			...
			<text x={ex + (cos >= 0 ? 1 : -1) * 12} y={ey} textAnchor={textAnchor} fill='#333'>{`Valor ${value.toLocaleString()}`}</text>
